...
aviobj = avifile('output.avi','fps',25,'COMPRESSION','None');
...
for i=1:10:NumberFrames
    ...
    if i~=1
        j=i-round(i/10);
%         disp(j);
        [B2,B3,P4,B5,B6,P7,B8,B9,I10]=GOP(j,FileName);
    elseif i==1
        j=i;
        [B2,B3,P4,B5,B6,P7,B8,B9,I10]=GOP(j,FileName);
    end
        %processing on P4 frame
        [bufferP4,streamP4,motionVect,PSNR,computations]=compensatedFrame(P4,bufferI1,mbSize,p,array,mynumber,threshold); 
        PSNRcalc(z)=PSNR;
...
% % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%        
        ...
        %for B2    
        [B2t ,motionVect, flag,computationsb,psnrb] = bFrameProc(B2,bufferI1,bufferP4,0.5*array,mbSize,p,mynumber,threshold);
        ...
        %for B3    
        [B3t ,motionVect ,flag,computationsb,psnrb] = bFrameProc(B3,bufferI1,bufferP4,0.5*array,mbSize,p,mynumber,threshold);
        computationscalc(z)=computationsb;
        PSNRcalc(z)=psnrb;
        z=z+1;
        c= makeLayers(B3t);
        ...
        [bufferP7,streamP7,motionVect,PSNR,computations]=compensatedFrame(P7,bufferP4,mbSize,p,array,mynumber,threshold);
        PSNRcalc(z)=PSNR;
        computationscalc(z)=computations;
        z=z+1;
        ...
        %for B5    
        [B5t motionVect flag,computationsb,psnrb] = bFrameProc(B5,bufferP4,bufferP7,0.5*array,mbSize,p,mynumber,threshold);
        computationscalc(z)=computationsb;
        PSNRcalc(z)=psnrb;
        z=z+1;
        c= makeLayers(B5t);
        ...
        %for B6    
        [B6t motionVect flag computationsb,psnrb] = bFrameProc(B6,bufferP4,bufferP7,0.5*array,mbSize,p,mynumber,threshold);
        computationscalc(z)=computationsb;
        PSNRcalc(z)=psnrb;
        z=z+1;
        c= makeLayers(B6t);
...
%       
        %disp(B8);
        %for B8    
        [B8t motionVect flag computationsb,psnrb] = bFrameProc(B8,bufferP7,bufferI10,0.5*array,mbSize,p,mynumber,threshold);
        computationscalc(z)=computationsb;
        PSNRcalc(z)=psnrb;
        z=z+1;
        c= makeLayers(B8t);
        ...
        %for B9    
        [B9t motionVect flag computationsb,psnrb] = bFrameProc(B9,bufferP7,bufferI10,0.5*array,mbSize,p,mynumber,threshold);
        computationscalc(z)=computationsb;
        PSNRcalc(z)=psnrb;
        z=z+1;
        c= makeLayers(B9t);
...
disp(z);
save dsplots7 PSNRcalc computationscalc
